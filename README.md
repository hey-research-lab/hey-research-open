...
| `packages/sdk` | `@hey-research/sdk` on npm: the typed client over the public API (projects, ships, signals, builders, the token lookup and the card call, bounties, weekly reports, chain activity), no dependencies, ESM and CJS, Node 18 or a browser. |
| `apps/mcp` | `@hey-research/mcp` on npm: the MCP server, twelve tools an assistant can call over the public API, started with `npx -y @hey-research/mcp`. |
...
```bash
npm i @hey-research/sdk            # the typed client, from npm
npx -y @hey-research/mcp           # the MCP server, from npm

pnpm install                       # or build both from this tree
...
lines. Releases of both packages are published from the private repository (tags `sdk-v*`,
