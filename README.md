...
| `packages/sdk` | The typed client over the public API (projects, ships, signals, builders, the token lookup and the card call, bounties, weekly reports, chain activity), no dependencies, ESM and CJS, Node 18 or a browser; it never follows a redirect, because the API key travels with the request. Build it from this tree: it is **not on npm yet** (see **Use** below). |
| `apps/mcp` | The MCP server, twelve tools an assistant can call over the public API (Node 20). `HEY_API_URL` must be https unless it is localhost: the API key rides on every request. Build it from this tree: **not on npm yet** either. |
...
Both packages are built and tested from this tree. **Neither is on npm yet** (checked
2026-09-19): the scope `@hey-research` has not been created, so `npm i @hey-research/sdk` and
`npx -y @hey-research/mcp` do not resolve. Build them from source:

```bash
pnpm install
pnpm lint && pnpm typecheck && pnpm test
pnpm --filter @hey-research/sdk build   # packages/sdk/dist
pnpm --filter @hey-research/mcp build   # apps/mcp/dist/index.js, the MCP server
```

Once the lab has created the npm organisation and the publish token, the same two packages
install as `npm i @hey-research/sdk` and run as `npx -y @hey-research/mcp`. Until then the
block above is the working path.
...
[docs/PUBLIC_API.md](docs/PUBLIC_API.md), and the SDK section there shows the client in ten
lines. Releases of both packages will be published from the private repository (tags `sdk-v*`,
`mcp-v*`); this repository mirrors their source.
...
(`docs/INTEGRATIONS.md` lives in the private repository and is not part of this export.)
...
is refused rather than rolled forward. A fourth pass (2026-09-18) changed, for an integrator: the logo proxy answers 404 for a host it does not allow instead of redirecting to it; `/api/search` and the signals page report the total their list is drawn from; `SCORING_VERSION` is `hbm-v7` (consistency counts ISO weeks, a comeback is measured from its cluster, and a project with no polled source reads UNKNOWN before QUIET); and an email address is confirmed by a button, never by a link preview. An old project slug that now belongs to a record HEY has withdrawn is not-found on the JSON API and a temporary redirect on the site and the badge; only a target still on the catalogue earns a permanent one. Since 2026-09-18 a keyed request draws on its tier's per-minute bucket from one address (it used to be capped at the anonymous limit); a key the lab has suspended, or an account it has blocked, answers `403 forbidden` with a `reason` (`key_suspended`, `account_suspended`, `account_blocked`), a revoked or expired key `401` like an unknown one; the monthly allowance is checked before a request is counted. `GET /api/v1/scan?chain=4663&token=0x…` (2026-09-18) is the same lookup in the shape a trading bot's card wants — `found`, status in HEY's words, `verified_builder`, commits/releases/ships in thirty days, the project page and a CTA to it; `found: false` is a 200 for an unpublished token or another chain. Since the sixth findings pass (2026-09-18): `commits_30d` counts only summaries that carry their days and is absent otherwise; any request carrying a key header is answered `private, no-store`; `/api/health/deep` is 503 when the worker has completed nothing for twenty minutes. The public API is also what the lab's own social copy reads: `scripts/social/fomo/numbers.sh` prints the day's figures from `/api/chain`, `/api/reports/weekly` and the listing totals, and a checker refuses a post that carries a number the API did not print that day. Since the seventh findings pass (2026-09-18): `SCORING_VERSION` is `hbm-v8` (code activity counts once per ISO week however many repositories produced it, Still Building is measured against the daily close and needs a market reading under a week old, and a pool is "liquidity removed" only under an absolute ceiling); `/api/signals` echoes only a `slug` it applied and orders with a stable tiebreaker; `/api/projects/{slug}/intelligence` names the market source the way `/market` does (`onchain`, never a supplier key); a 404 from `/api/badge` has the same `{error: 'not_found', message}` shape as every other route; the MCP tools carry `stillBuildingEvidence` and accept a `0X` address prefix; and `llms.txt` states the single-token distribution boundary. Since the eighth findings pass (2026-09-18): `commits_30d` on the card call may carry `commits_30d_partial: true`, meaning HEY read a cut page of commits inside the window and the figure is a floor (print `100+` or drop it); a commit summary read from a full page is titled "100+ commits since <date>" rather than an exact ninety-day count; a 429 from a provider is honoured for exactly the `Retry-After` it names and never retried in-process; a feed entry dated more than ten minutes ahead is skipped; and the free API tier's monthly ceiling is a console setting (default 10,000). A full-platform audit on 2026-09-19 (eleven auditors, every finding proved against production) repaired eighty-nine things; for an integrator the ones that matter are: a project's commit summaries are no longer duplicated by a week-key casing split, signals never announce a release the project page has withdrawn, the development signals count meaningful ships rather than bare contract deploys, a published project with no token redirects instead of 404ing on its market page, `/api/signals` documents the four kinds that count addresses, and the API documentation now states the single-token distribution boundary rather than denying it. The lab's console gained an analytics section on 2026-09-18 (traffic, product use, project attention, unmet demand from searches and scans, Search Console when a credential is set, a daily technical-SEO audit); it reads only first-party rows and Google's own API, stores no IP, user agent or cross-day identifier, and none of it reaches any public figure or ranking. Since 2026-09-19 the whole API is one typed client, `@hey-research/sdk` (`packages/sdk` here): `new HeyClient().scanCard(4663, address)` is the card call with its shapes, `projects.items({ tab: 'still-building' })` walks a listing, a `429` surfaces as `HeyApiError` with `retryAfterSeconds` and is never retried for you, and a type-level contract test in the private repository fails the gate when the API and the client disagree on a single field.
