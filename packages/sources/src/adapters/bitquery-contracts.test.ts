...
describe('bitquery contract activity', () => {
  it('asks for counts and never for an account', () => {
    ...
    // account on this chain. The query must not, and this is the guard.
    expect(BITQUERY_CONTRACT_ACTIVITY_QUERY).not.toMatch(/Transaction_From|Call_From|Transfer_Sender|Holder|Balance/);
  ...
  it('joins three cubes into one row per contract and day', () => {
    const rows = normalizeBitqueryContractDays({
      calls: [
        { Block: { Date: '2026-09-13' }, Call: { To: A.toUpperCase() }, calls: '1450750', methods: '17' },
        { Block: { Date: '2026-09-12' }, Call: { To: A }, calls: '900', methods: '4' },
      ],
      events: [
    ...
    expect(latest).toEqual({ address: A, day: '2026-09-13', calls: 1_450_750, methods: 17, events: 590_679, eventKinds: 3, transactions: 4_200 });
    // A contract that only emitted events still gets a row, with zeroes for the rest.
    expect(rows.find((row) => row.address === B)).toEqual({ address: B, day: '2026-09-13', calls: 0, methods: 0, events: 12, eventKinds: 2, transactions: 0 });
  });

  it('drops a malformed address or day rather than guessing', () => {
    const rows = normalizeBitqueryContractDays({
      calls: [
        { Block: { Date: 'not-a-day' }, Call: { To: A }, calls: '5', methods: '1' },
        { Block: { Date: '2026-09-13' }, Call: { To: '0xnope' }, calls: '5', methods: '1' },
        { Block: { Date: '2026-09-13' }, Call: { To: null }, calls: '5', methods: '1' },
      ],
      events: [],
      transactions: [],
    });
